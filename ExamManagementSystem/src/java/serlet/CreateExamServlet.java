...
public class CreateExamServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            ps.setString(1, request.getParameter("exam_title"));
            ps.setString(2, request.getParameter("subject"));
            ps.setInt(3, Integer.parseInt(request.getParameter("category_id")));
            ps.setInt(4, Integer.parseInt(request.getParameter("total_marks")));
            ps.setInt(5, Integer.parseInt(request.getParameter("duration")));
            ps.executeUpdate();
            con.close();
            response.sendRedirect("dashboard.jsp?msg=Exam Created Successfully");
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
